...
@section('content')
    <div>
        <div class="col-xs-10 col-md-10 col-lg-10 col-xl-10 offset-md-1 alignTop ">
            <h1 class="h1 text-center">Articles liés au tag <span class="text-info">{{$tag->name}}</span></h1>
            ...
            <div class="marginBottomAjoutArticle tagsLink">
                @foreach($tags as $tag)
                    <button class="btn tag_{{$tag->color}}"><a href="{{route('admin.tags_assoc_posts', $tag->id)}}">{{$tag->name}}</a></button>
                @endforeach
            </div>
            <div class="row">
